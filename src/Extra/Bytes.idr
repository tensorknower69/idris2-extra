...
interface ByteArray (f : Nat -> Type) where
  ...
  ||| Allocate/create a `ByteArray` from a `Vect` of `Bits8`
  pack : LinearIO io => {size : Nat} -> Vect size Bits8 -> L io {use=1} (f size)
  ||| Extract a `Vect` of `Bits8` from a `ByteArray`
